...
public interface IUtilisateurDAO extends JpaRepository<Utilisateur, Integer>{
     ...
     * @return
     */
    @Query("select user from Utilisateur user join fetch user.comments as cmt join fetch user.announcements as an where cmt.creationDate >=:paramDatePivot and an.endCity like %:paramEndCity%")
    public List<Utilisateur> findAuthorsCommentByDateAndPostedAnnonce(@Param("paramDatePivot") Date paramDatePivot,@Param("paramEndCity") String paramEndCity);
     ...
     */
    @Query("select user from Utilisateur user where user.email like %:paramDomaine and user.phone is not null")
