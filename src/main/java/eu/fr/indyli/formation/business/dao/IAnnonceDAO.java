...
public interface IAnnonceDAO extends JpaRepository<Annonce, Integer>{
	...
	@Modifying
	@Query("update Annonce an set an.prime = an.prime * 2  where an.startCity like CONCAT('%',:paramDepTown,'%')")
	 ...
	 * @return
	 */
	@Query("Select an From Annonce an where an.startCity like CONCAT('%',:paramDepTown,'%')")
