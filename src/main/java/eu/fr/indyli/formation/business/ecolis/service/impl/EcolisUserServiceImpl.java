...
public class EcolisUserServiceImpl extends AbstractServiceImpl<EcolisUser, EcolisUserBasicDTO, EcolisUserFullDTO, IEcolisUserDAO> implements IEcolisUserService, UserDetailsService {
	...
	public IEcolisUserDAO getDAO() {
	  ...
	  @Override
	  public EcolisUser findByLoginAndPassword(String email, String password)
	  ...
	  @Override
	  public List<EcolisUser> findAuthorsCommentByDateAndPostedAnnonce(Date paramDatePivot,
	  ...
	  public UserDetails loadUserByUsername(String login) throws UsernameNotFoundException {
	    ...
	    return new org.springframework.security.core.userdetails.User(ecolisUser.getLogin(),
	        ecolisUser.getPassword(), getAuthority());
	  }
	  ...
	  @Override
	  public List<EcolisUser> findByLoginOrEmail(String login, String email)
	      throws EcolisBusinessException {
	...
	public EcolisUser create(EcolisUser user) throws EcolisBusinessException {
		// On verifie si l'email et le login n'existent pas déjà en base
		List<EcolisUser> listeUserExistantEnBaseAvecLoginOuEmail = this.userDAO.findByLoginOrEmail(user.getLogin(), user.getEmail());
		if (!listeUserExistantEnBaseAvecLoginOuEmail.isEmpty()) {
	      throw new EcolisBusinessException("Le login :" + user.getLogin() + " Ou L'email :"
	          + user.getEmail() + " semblent déjà pris");
		}
